...
if "ver_pantalla_tv" in st.session_state and st.session_state.ver_pantalla_tv:
    ...
    # ==========================================================
    # 🚨 RECEPTOR DE ALERTA: INTERRUMPE LA PANTALLA SI HAY EMERGENCIA
    # ==========================================================
    import json
    import os
    import datetime as dt
    ...
    if os.path.exists(archivo_alerta):
        try:
            ...
            if dt_datetime.now() < expiracion_alerta:
                # 1. Dibujamos la pantalla roja gigante
                st.markdown(f"""
                    <style>
                ...
                # 2. Hacemos sonar la alarma local (solo 1 vez)
                id_unica_alerta = alerta_data["expiracion"] 
                if st.session_state.get("ultima_alerta_sonada") != id_unica_alerta:
                ...
                # 3. FRENAMOS AQUÍ PARA QUE NO SE DIBUJE EL COLEGIO DE FONDO
                st.stop() 
            else:
                os.remove(archivo_alerta)
        except Exception as e:
    ...
    with col_ann:
        ...
        # 🔔 GESTOR DE SONIDOS DE TV NORMAL (ARCHIVOS LOCALES)
        try:
    ...
    # Frenamos todo al final de la pantalla TV
    st.stop()
